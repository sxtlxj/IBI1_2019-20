...
os.chdir("D:\computer\IBI1_2019-20\Practical7") # open the file
covid_data = pd.read_csv("full_data.csv")
my_columns = [True, True, False, True, False,False] # decide which columns to show
print(covid_data.iloc[:,0:15:3])

my_rows = []
for i in range(0,7996):
    if covid_data.loc[i,"location"] == "Afghanistan": #select the data in Afghanistan
        my_rows.append(True)
    else:
...
my_rows3 = []
for i in range(0,7996):
    if covid_data.loc[i,"location"] == "World": #select the data of World
        my_rows1.append(True)
    else:
        my_rows1.append(False)
world_new_cases = covid_data.loc[my_rows1,"new_cases"]#select the data of new cases
...
world_new_deaths = covid_data.loc[my_rows1,"new_deaths"]#select the data of new deaths
...
print(median,"\n",mean) #show the median and mean of the world new cases

plt.plot(world_date, world_new_cases, 'ro') #draw the plot with lines in different colors
...
rowtotal = []
for i in range(0,7996):
    if covid_data.loc[i,"location"] == "Spain": #use the same way above to find data of Spain
        rownew.append(True)
    else:
        rownew.append(False)
